...
Metro_Hast.R <- function (lambda_start,rho_start,t_start,burn.iter,iter,a,x) {
  lambda=rep(NA, burn.iter+iter) 
  lambda[1]=lambda_start
  rho=rep(NA, burn.iter+iter)
  rho[1]=rho_start
  t=rep(NA, burn.iter+iter)
  t[1]=t_start
  loglik=rep(NA, burn.iter+iter)
  ...
  accept=rep(NA, burn.iter+iter)
  accept[1]=0
  
  for (i in 2:burn.iter) {
    prop.sd <- c(0.001,0.5,10)
  ...
  p_cur=sum(accept[1:burn.iter])/burn.iter
  prop.sd_new <- cov(cbind(lambda[1:burn.iter],rho[1:burn.iter],t[1:burn.iter]))*qnorm(0.238/2, mean=0, sd=1)/qnorm(p_cur/2)
  
  for (i in (burn.iter+1):(burn.iter+iter)) {
    mu <- c(lambda[i-1], rho[i-1], t[i-1])
    theta.new <- mvrnorm(1,mu,prop.sd_new )
...
start_time <- Sys.time()
R.try <- Metro_Hast.R(lambda_start, rho_start, t_start, burn.iter, iter, a,x)
end_time <- Sys.time()
end_time - start_time 
nrow(R.try)
plot(R.try$lambda)
...
sourceCpp("Metro_Hast_adaptive.cpp")

sourceCpp("debug_adaptive_1.cpp")


start_time <- Sys.time()
try.cpp <- Metro_Hast_cpp(lambda_start, rho_start, t_start, burn.iter, iter, a,x)
end_time <- Sys.time()
end_time - start_time 
